          ...
          rank.gene.nudgey = 2) 
{
  if(is.null(curveCol)){
    ...
    # curveCol <- RColorBrewer::brewer.pal(length(geneSetID), "Paired")
    curveCol <- colorRampPalette(colors = RColorBrewer::brewer.pal(12, "Paired"))(length(geneSetID))
  }
  ...
  niceTit <- purrr::map_chr(unique(gsdata$Description), function(x) {
    ...
    if (length(tit) == 1) {
      niceTit <- paste(tit[1:length(tit)], 
                       collapse = " ") %>% stringr::str_wrap(., width = termWidth)
    }
    else {
      if (rmPrefix == TRUE) {
        niceTit <- paste(tit[2:length(tit)], 
                         collapse = " ") %>% stringr::str_wrap(., width = termWidth)
      }
      else {
        niceTit <- paste(tit[1:length(tit)], 
  ...
  if (length(geneSetID) == 1) {
    line <- ggplot2::geom_line(
                              #  ggplot2::aes_(color = ~runningScore), 
                               size = lineSize)
