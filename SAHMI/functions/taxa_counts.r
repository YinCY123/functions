...
print("Checking for NAs and empty strings in barcode and taxid:")
print("Barcode summary:")
print(table(nchar(s.bc$barcode)))
print("Taxid summary:")
print(table(nchar(s.bc$taxid)))

# Check for empty strings
empty_bc <- sum(s.bc$barcode == "")
empty_tax <- sum(s.bc$taxid == "")
print(paste("Number of empty strings in barcode:", empty_bc))
print(paste("Number of empty strings in taxid:", empty_tax))

# Print unique values
print(paste0("Number of unique barcodes: ", length(unique(s.bc$barcode))))
print(paste0("Number of unique taxids: ", length(unique(s.bc$taxid))))

# Check if barcode and taxid are factors
print("Converting barcode and taxid to factors...")
s.bc$barcode <- as.factor(s.bc$barcode)
